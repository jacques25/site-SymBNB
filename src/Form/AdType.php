    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        $builder
            ->add(
                ...
                $this->getConfiguration("Titre", "Tapez le titre de votre annonce")
            )
            ->add(
                'slug',
                TextType::class,
                $this->getConfiguration("Chaine URL", "Adresse web(automatique)", $options = [
                    'required' => false
                ])
            )
            ->add(
                'coverImage',
                UrlType::class,
                $this->getConfiguration("url de l'image principale", "donnez une adresse d'une image qui donne envie")
            )
            ->add(
                'introduction',
                TextType::class,
